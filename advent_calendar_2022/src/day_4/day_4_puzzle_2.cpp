...
int check_for_overlap(vector<int> group_one, vector<int> group_two){
    ...
    if(includes(container_one.begin(), container_one.end(), container_two.begin(), container_two.end())){
        return true;
    }
    ...
    if (group_one[0] > group_two[0]){
        ...
        {
            int lower = 0;
            int upper = group_one.size() - 1;
            int mid = group_one.size() / 2;
            while (lower <= upper){
                mid = lower + (upper-lower) / 2;
                if (group_two[group_two.size()-1] == group_one[mid]){
                    overlaps = true;
                    return overlaps;
                }
                else if (group_two[group_two.size()-1] > group_one[mid])
                {
                    lower = mid + 1;
                }
                else {
        ...
        for (int i = 0; i < group_two.size(); i++){
            int lower = 0;
            int upper = group_two.size() - 1;
            int mid = group_two.size() / 2;
            while (lower <= upper){
                mid = lower + (upper-lower) / 2;
                if (group_one[group_one.size()-1] == group_two[mid]){
                    overlaps = true;
                    return overlaps;
                }
                else if (group_one[group_one.size()-1] > group_two[mid])
                {
                    lower = mid + 1;
                }
                else {
...
int test_does_not_overlap_or_contain(){
    vector<int> group_one = {2,3};
    vector<int> group_two = {4,5};
    return !check_for_overlap(group_one, group_two);
}
int test_overlap_beginning(){
    vector<int> group_one = {3,4,5,6,7};
    vector<int> group_two = {1,2,3};
    return check_for_overlap(group_one, group_two);
}
int test_overlap_end(){
    vector<int> group_one = {3,4,5,6,7};
    vector<int> group_two = {6,7,8,9,10};
    return check_for_overlap(group_one, group_two);

}

int test_overlap_low_long(){
    vector<int> group_one = {2,3,4,5,6,7,8,9,10};
    vector<int> group_two = {3,4,5,6,7,8,9};
    return check_for_overlap(group_one, group_two);
}
...
int main(){
    ...
    assert(test_does_not_overlap_or_contain());
    assert(test_overlap_beginning());
    assert(test_overlap_end());
    assert(test_overlap_low_long());
